...
def download_tin(work_dir):
		...
		for directory in tqdm(os.listdir(path)):
			if (path.name == 'train'):
				...
				for file in os.listdir(os.path.join(path_to_dir,  'images')):
					shutil.move(os.path.join(path_to_dir, "images", file), os.path.join(dest_path, file))

				os.rmdir(os.path.join(path_to_dir, "images"))
	...
	path_to_file = os.path.join(work_dir, "Data")
	...
	path_to_dataset = os.path.join(path_to_file, "tiny-imagenet-200")

	#Prep the Data now
	file_list = ['train', 'test', 'val', 'wnids.txt', 'words.txt']

	for file_sp in tqdm(file_list):
		convert_tiny_imagenet(os.path.join(path_to_dataset, file_sp))
